...
(define-public (bestow-laureates)
    (begin
        ...
        (print (map transfer-stx (try! (get-laureates-list "A"))))
        (var-set stxShare (try! (get-laureates-stx-share "B")))
        (print (map transfer-stx (try! (get-laureates-list "B"))))
        (var-set stxShare (try! (get-laureates-stx-share "C")))
        (print (map transfer-stx (try! (get-laureates-list "C"))))
        (var-set stxShare (try! (get-laureates-stx-share "D")))
        (print (map transfer-stx (try! (get-laureates-list "D"))))

        (var-set ftShare (try! (get-laureates-ft-share "A")))
        (print (map transfer-mr-token (try! (get-laureates-list "A"))))
        (var-set ftShare (try! (get-laureates-ft-share "B")))
        (print (map transfer-mr-token (try! (get-laureates-list "B"))))
        (var-set ftShare (try! (get-laureates-ft-share "C")))
        (print (map transfer-mr-token (try! (get-laureates-list "C"))))
        (var-set ftShare (try! (get-laureates-ft-share "D")))
        (print (map transfer-mr-token (try! (get-laureates-list "D"))))
